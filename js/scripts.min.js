...
$(document).ready(function() {
	...
	$('.hero-banner').slick({
        ...
        customPaging: function () {
            return '<a class="slick-dots__link" data-role="none" role="button" aria-required="false" href="#"></a>'
        }
    });

	$('.info-item__slider-image').slick({
        dots: true,
        arrows: false,
        autoplay: true,
        autoplaySpeed: 2000,
    ...
    $(window).scroll(function(){
        let sticky = $('nav'),
            sticky_m = $('.action-block-mobile'),
            scroll = $(window).scrollTop(),
            scroll_top = $(".scroll-top-container");

        if (scroll >= 90) sticky_m.addClass('sticky');
        else sticky_m.removeClass('sticky');
        if (scroll >= 205) sticky.addClass('sticky');
    ...
    //респонсив в категориях если есть фильтры то блок с инфой переносится вниз
    if ($(window).width() < 992 && $("div").is(".info-block__filters")
        && !$("div").is(".info-block__additional")) {
        $('.main-content').append('<div class="info-block__additional">');
        $('.info-block__additional').append( $('.info-block__filters'))
    }

    $(window).resize(function () {
        if ($(window).width() < 992 && $("div").is(".info-block__filters")
            && !$("div").is(".info-block__additional")) {
            $('.main-content').prepend('<div class="info-block__additional">');
            $('.info-block__additional').prepend( $('.info-block__filters'))
        }
        if ($(window).width() > 992 && $("div").is(".info-block")
            && $("div").is(".info-block__additional")) {
            $('.info-block').prepend( $('.info-block__filters'))
            $('.info-block__additional').remove();
        }
        menuWidth()
    });
    ...
    $(".category-list .category-list__item").hover(
        function () {
            if (window.innerWidth > 1260) {
                $(this).parent().find('.category-list__link').removeClass("hover");
                $(this).find('> .category-list__link').addClass("hover");
            }
        }
    );
    $(".category-list .category-list__child-button").click(
        function (e) {
    ...
    function menuWidth() {
        if (window.innerWidth > 1260) {
            let quarter = (($(window).width() - 1280) / 2) / $(window).width() * 100
            let col = 315 / $(window).width() * 100
            $('#catalog-modal .catalog-content .lvl-0-wrapper').css("left", quarter + "%")
            $('#catalog-modal .catalog-content .lvl-1-wrapper').css("left", quarter + col + "%")
            $('#catalog-modal .catalog-content .lvl-2-wrapper').css("left", quarter + col * 2 + "%")
            $('#catalog-modal .catalog-content .lvl-3-wrapper').css("left", quarter + col * 3 + "%")
        }
    }
    menuWidth()

    //При клике на кнопку развертываемой модалки открывать ее
    $( ".slide-modal-button" ).click(function() {
        $('.slide-modal').removeClass('slide-modal-open')
        $(this).parent().find('.slide-modal').toggleClass('slide-modal-open')
    });
    //При клике куда угодно кроме кнопки если развертываемая модалка открыта закрыть её (или дропдаун)
    $(document).click(function(e) {
        if ( $(e.target).closest('.slide-modal-button').length === 0
            && $(e.target).closest('.slide-modal').length === 0
            && $(e.target).closest('.slide-search input').length === 0) {
            $('.slide-modal').removeClass('slide-modal-open')
        }
        if( $(e.target).closest('.dropdown__menu').length === 0 &&
            $(e.target).closest('.dropdown__label').length === 0){
            $('.dropdown').removeClass('open')
        }
    });

    $('.slide-search input').focus(function()
    {
        $('.slide-modal').removeClass('slide-modal-open')
        $('.slide-search').parent().find('.slide-modal').addClass('slide-modal-open')
    });

    //Маска для телефонов
    for (let i = $(".phone-mask").length - 1; i >= 0; i--) {
        $(".phone-mask").eq(i).mask("+7 (999) 999-99-99")
    }

    $('.m-search-wrapper .search-form__button').click(
        function (e) {
            e.preventDefault();
            $('.m-search-wrapper').addClass("active")
        }
    )
    $('.m-search-wrapper .search-form__close').click(
        function (e) {
            e.preventDefault();
            $('.m-search-wrapper').removeClass("active")
        }
    )

    //фильтры, анимация раскрытия
    $('.filter-item__content').hide();

    $('.filter-item__header').click(function() {
        if ($(this).parent().hasClass("open")) {
            $(this).parent().find(".filter-item__content").slideUp( 300, function() {
                $(this).parent().removeClass("open");
            });
        } else {
            $(this).parent().addClass("open").find(".filter-item__content").slideDown(300);
        }
        return false;
    });


    $('.dropdown .dropdown__label').click(
        function () {
    ...
    $('.product-gallery__nav').slick({
        ...
        slidesToScroll: 1,
        prevArrow:
            '<a class="gallery-arrow gallery-prev" href="#">\n' +
                '<svg width="12" height="21" viewBox="0 0 12 21" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '    <path d="M12 18.515L4.583 10.5L12 2.4675L9.7166 0L0 10.5L9.7166 21L12 18.515Z" fill="#252726"/>\n' +
                '</svg>'+
            '</a>',
        nextArrow:
            '<a class="gallery-arrow gallery-next" href="#">\n' +
                '<svg width="12" height="21" viewBox="0 0 12 21" fill="none" xmlns="http://www.w3.org/2000/svg">\n' +
                '    <path d="M0 18.515L7.417 10.5L0 2.4675L2.2834 0L12 10.5L2.2834 21L0 18.515Z" fill="#252726"/>\n' +
                '</svg>'+
            '</a>',
    ...
    if(localStorage.getItem('hide-subcategory') === "true"){
        $(".category.switchable").hide();
    }

    $(".category-switch a").on('click', function(e) {
        e.preventDefault();
        if(localStorage.getItem('hide-subcategory') === "true"){
            localStorage.setItem('hide-subcategory', "false");
            $(".category.switchable").slideDown(500, function () {
                $(".category-switch a").text("Скрывать подкатегории")
            })
        }else {
            localStorage.setItem('hide-subcategory', "true");
            $(".category.switchable").slideUp(500, function () {
                $(".category-switch a").text("Показывать подкатегории")
            })
        }
    });

    let loader = $(
        "<div class=\"folding-loader\">" +
